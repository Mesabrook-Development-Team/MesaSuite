...
namespace API_Government.Controllers
{
    ...
    public class EmailImplementationController : DataObjectController<EmailImplementation>
    {
        ...
        private long? CompanyID => long.TryParse(Request.Headers.FirstOrDefault(kvp => kvp.Key.Equals("CompanyID", StringComparison.OrdinalIgnoreCase)).Value?.FirstOrDefault(), out long governmentID) ? (long?)governmentID : null;
        private long? GovernmentID => long.TryParse(Request.Headers.FirstOrDefault(kvp => kvp.Key.Equals("GovernmentID", StringComparison.OrdinalIgnoreCase)).Value?.FirstOrDefault(), out long governmentID) ? (long?)governmentID : null;
        ...
        public override async Task<EmailImplementation> Get(long id)
        {
            ...
            if (!template.SecurityCheck(SecurityProfile.UserID, CompanyID, null, GovernmentID))
            {
                return null;
            }
        ...
        public override async Task<IHttpActionResult> Post(EmailImplementation dataObject)
        {
            ...
            if (emailTemplate == null || !emailTemplate.SecurityCheck(SecurityProfile.UserID, CompanyID, null, GovernmentID))
            {
        ...
        public override IHttpActionResult Delete(long id)
        {
            ...
            if (implementation == null || !implementation.EmailTemplate.SecurityCheck(SecurityProfile.UserID, CompanyID, null, GovernmentID))
            {
                return Unauthorized(new AuthenticationHeaderValue[0]);
            }
